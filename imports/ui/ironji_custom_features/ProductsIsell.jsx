...
import { Product_i_sell_item } from './Product_i_sell_item';




export class ProductsIsell extends Component {
    constructor(props) {
        super(props);
        this.state = {
            productsIsell:[],
    ...
    searchInProductsUsell() {
        document.getElementById("fromDatabase__").innerHTML = "Loading...";

        fetch('https://map.ogeniuspriority.com/map_scripts/search_products_from_my_stocks.php?id=' + global.the_id_op + "&productName=" + document.getElementById("ProductIsellToSearch").value)
            .then(response => response.json())
            .then(resData => {
                TheTradersData = JSON.parse(JSON.stringify(resData));
                var theMarkersOfTraders = TheTradersData["theMarkersOfTraders"];
                ...
                var theResults1 = [];
                    ...
                    this.setState({ productsIsell: theResults1 });
                ...
                for (var key in theMarkersOfTraders) {
                    if (theMarkersOfTraders.hasOwnProperty(key)) {
                        ...
                        theResults1.push(theMarkersOfTraders[key].ironji_traders_product_list_id + "~" + theMarkersOfTraders[key].ironji_traders_product_list_regdate + "~" + theMarkersOfTraders[key].ironji_traders_product_list_name + "~" + theMarkersOfTraders[key].ironji_traders_product_list_creator + "~" + theMarkersOfTraders[key].ironji_traders_product_list_type_kind + "~" + theMarkersOfTraders[key].ironji_traders_product_list_image + "~" + theMarkersOfTraders[key].ironji_traders_product_list_add_user_price);
                        i_db++;
                    }
                }
                //----------------
                document.getElementById("fromDatabase__").innerHTML = "";
    ...
    showResults() {
        ...
        if (this.state.productsIsell) {
            return (this.state.productsIsell.map((el) => (


                <Product_i_sell_item id_prod={el.split("~")[0]} regdate_prod={el.split("~")[1]} name_prod={el.split("~")[2]} creator_prod={el.split("~")[3]} type_kind={el.split("~")[4]} image_prod={el.split("~")[5]} price_prod={el.split("~")[6]} ><span>{} </span></Product_i_sell_item>
            )));
        } else {
    ...
    render() {
        ...
        return (<div>
            <div style={{ background: "#ebedee", borderRadius: "4px", padding: "5px", width: "120%", border: "1px solid #fff", marginLeft: "1%" }}>
                <h4>View products you sell!:</h4>
                <div className="md-form mt-0">
                    <input id="ProductIsellToSearch" onKeyUp={this.searchInProductsUsell.bind(this)} className="form-control" type="text" style={{ width: "300px" }} placeholder="When empty all the products are shown" aria-label="Search" />
                    <input onClick={this.searchInProductsUsell.bind(this)} type="button" className="btn-primary" value="Search" />
                    <div id="fromDatabase__"></div>
